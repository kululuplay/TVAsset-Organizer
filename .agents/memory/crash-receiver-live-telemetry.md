...
**Portal username (panel "Kullanıcı") = account↔device mapping:** the heartbeat sends the IPTV portal login `username` (from `SettingsStore.getSourceConfig()?.username`, NEVER the password), shown as a panel column. Two durable lessons:
- **Only send it when present.** M3U-URL sources have no login and logged-out boxes have none, so the app gates on `isNotBlank()` and the field is simply omitted from the JSON.
- **The upsert must PRESERVE-on-absent, not overwrite.** Heartbeats fire every ~60s; if a beat that omits username (M3U / logged-out) did a plain `username = EXCLUDED.username` it would wipe the stored value once a minute and the column would be useless. Use `username = COALESCE(EXCLUDED.username, devices.username)` AND coerce a blank/whitespace username to NULL server-side (so a blank beat is treated as "absent", not "clear"). An account *switch* still updates because the new non-null username overrides. Accepted tradeoff: a stale username lingers after logout until the next non-null beat.
