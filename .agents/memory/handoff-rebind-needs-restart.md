...
**Race guard:** `rebind()`'s detach->delay->attach posts on the shared main handler, so a rapid back-and-forth hand-off (preview->fullscreen->BACK inside the swap-delay window) can stack two delayed re-attaches that both fire. Reuse the existing `startGeneration` token: bump it in `rebind()`, capture it, and in the delayed runnable bail if `generation != startGeneration || eng !== engine`. A `play()` in between is already safe because it clears the handler.
