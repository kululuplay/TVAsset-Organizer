...
object PlaybackSupportReporter {
    ...
    fun capture(rows: List<Map<String, Any>>, boundary: Boolean = true) {
        if (rows.isEmpty() && boundary) return
        if (app == null) return
        captures.trySend(Capture(rows.take(16), boundary, System.currentTimeMillis()))
    }
    ...
    private suspend fun persistCapture(item: Capture) {
        ...
        try {
            ...
            mutex.withLock {
                ...
                queue.enqueue(PlaybackUploadQueue.Entry(id, body, item.sampledAt, item.boundary))
                persist()
            }
        ...
        loop = scope.launch {
            while (foreground) {
                ...
                try {
                    ...
                    // At most two requests per 30s: latest state, then one retained boundary.
                    for (attempt in 0..1) {
                ...
                delay((30_000L - (SystemClock.elapsedRealtime() - cycleStartedAt)).coerceAtLeast(1_000L))
            }
        }
    }
    ...
    private fun load(context: Context) {
        ...
        if (file.length() <= 512 * 1024) runCatching {
            ...
            val entries = (0 until minOf(rows.length(), 32)).map { index ->
                ...
                PlaybackUploadQueue.Entry(row.getString("id"), row.getString("body"), row.getLong("at"), row.getBoolean("boundary"))
            }
    ...
    private fun persist() {
        val json = JSONObject().put("failures", queue.failureCount).put("next", queue.nextAttemptMs)
            .put("queue", JSONArray(queue.entries().map { entry -> JSONObject().put("id", entry.id)
                .put("body", entry.body).put("at", entry.sampledAtMs).put("boundary", entry.boundary) }))
