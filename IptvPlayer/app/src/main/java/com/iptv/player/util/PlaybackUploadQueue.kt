...
internal class PlaybackUploadQueue(
    ...
    data class Entry(val id: String, val body: String, val sampledAtMs: Long, val boundary: Boolean)
    ...
    fun next(now: Long): Entry? {
        ...
        // Returning online must show the current state before draining old boundaries.
        return items.lastOrNull { !it.boundary } ?: items.firstOrNull()
    }
