...
assert "name='com.iptv.player.tstest'" in badging
assert "versionName='1.5.86-ts-test1'" in badging
assert "sdkVersion:'21'" in badging
assert "application-label:'Kululu IPTV TS Test'" in badging
...
result = out/'KululuIPTV-1.5.86-ts-test1.apk'
...
metadata = {'apk':result.name, 'bytes':result.stat().st_size, 'sha256':digest, 'package':'com.iptv.player.tstest', 'version':'1.5.86-ts-test1', 'abis':abis, 'tests':counts, 'test_classes':len(suites), 'lint':lint_counts, 'media3_hls_classes':False, 'commit':os.environ.get('GITHUB_SHA'), 'real_device_test':'Pending user Android stick test'}
