...
- Capacity is bounded transactionally: 100,000 installations, 100,000 tickets, 10,000 retained log attachments at <=128 KiB each. Daily global quotas and per-IP/per-device limits supplement those caps. A full ticket store returns a 503 error, never a false success. Operators should export/archive old tickets before caps are reached. Playback history caps are rings that drop the oldest rows instead (see below); the only playback refusal is a new device beyond the 25,000-device cap.
- Rate-limit tables are bounded (10,000 keys, plus a separate 1,000-key table for operator sign-in). A full table evicts expired keys, then the earliest-expiring one, so a new device or a new operator address is never refused merely because the table is full; existing keys keep their counts.
...
- A fresh report is at most two minutes old while the last snapshot carries an unfinished session, considering both source and receipt time; a snapshot with no unfinished session (idle Home screen or a delivered final boundary) stays fresh for six minutes, slightly above the idle refresh interval. The server sizes the window per device (`freshWindowMs`, also applied to the online/problem counts) and the dashboard follows it. Missing frame counters, paused/ended sessions, stale reports and clock skew never imply healthy video. Diagnoses describe measured evidence and suggested checks; they cannot establish that a panel or Wi-Fi caused a failure on their own.
- Timeline retention is **up to** seven days, 2,000 samples per device and 100,000 globally; capacity can shorten history. Detail shows the latest 100 entries and discloses limits. Retry receipts are rings too: 500 newest per device and 1,000,000 fleet-wide. A retry is acknowledged while either its receipt or its timeline row survives; an old replay that outlives both is stored as ordinary late history. Late replay cannot replace a newer current state or reopen a finalized session.
- Fleet-wide playback caps (devices, samples, receipts, incidents) are counted and pruned at most once a minute per process, outside the upload path; uploads take only a per-installation lock and bounded per-device index lookups, so ticket writes and other devices are never serialised behind them. A device clock ahead of the server is clamped to receipt time on ingest (the envelope keeps its own timestamp in the stored payload), so one skewed envelope cannot hide later measurements.
...
- Incidents and intervention notes have seven-day retention and ring caps: 50,000 incidents fleet-wide (pruned by the minute check) / 200 per device (pruned on upload); 10,000 interventions / 100 per device (checked transactionally when a marker is created). A comparison with no measurement in either window says so instead of reporting a content mismatch. Detail displays the latest 30 incidents and 10 interventions, with truncation disclosed. Before/after results retain bounded baseline evidence and freeze only after the measurement window closes.
...
By default PostgreSQL tests are skipped locally; set `SUPPORT_TEST_DATABASE_URL` to an isolated test database to include them. On the VPS the verification script supplies the dedicated database URL without printing credentials and tests only temporary schemas: it first proves the server honours the `search_path` connection option with a throwaway probe schema and refuses to run otherwise, and every database test asserts `current_schema()` is its own random schema and qualifies its bulk fixture mutations with that schema and its fixture installation ids. After Android unit tests, run `node scripts/verify_support_contract.js` from the repository root to validate real Kotlin-produced envelopes against the server. Real Android/Fire TV end-to-end and long-duration playback validation require actual devices; server and unit tests do not establish device playback stability.
