...
async function fixture(t) {
  ...
  const store = { authenticate: async (candidate, h) => candidate === id && h === hash(secret), playback: async (owner, sample) => { writes.push({ owner, sample }); return sample.sampleId; }, playbackList: async filter => ({ items: [], filter }), playbackDetail: async () => null };
  ...
  return { call, writes, id, device: { Authorization: `Bearer ${id}.${secret}` }, admin: { Cookie: login.headers.get("set-cookie").split(";")[0] } };
}
