...
// The server sizes each card's freshness window to the client cadence (two minutes while a session is active, six minutes for an idle card).
function healthWindow(item) { return Number.isFinite(item?.freshWindowMs) ? item.freshWindowMs : HEALTH_STALE_MS; }
...
function healthState(item, failed = healthError) { return !failed && item?.online === true && healthAge(item) <= healthWindow(item) ? (["problem", "healthy"].includes(item.status) ? item.status : "unknown") : "unknown"; }
function healthStateName(item, failed = healthError) { const state = healthState(item, failed); if (state === "problem") return "Sorun sinyali"; if (state === "healthy") return "Sorun sinyali yok"; if (failed) return "Bağlantı doğrulanamadı"; return healthAge(item) > healthWindow(item) ? "Güncel ölçüm yok" : "Yetersiz ölçüm"; }
...
function renderHealthRows() {
  ...
  for (const item of healthRows) {
    ...
    const content = element("div", null, "health-current"); content.append(element("span", session?.final === false ? "SON ÖLÇÜMDE OYNATMA" : "SON OYNATMA OTURUMU", "eyebrow"), element("strong", sessionName(session)), element("span", sessionState(session, healthError || !item.online || healthAge(item) > healthWindow(item)), "device-version"));
