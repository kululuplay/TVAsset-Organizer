...
const enums = {
  ...
  end_reason: sets("USER_STOP COMPLETED REPLACED BACKGROUND FATAL_FAILURE APP_SHUTDOWN"), state: states,
};
const lists = {
...
const numbers = sets("started_at_epoch_ms ended_at_epoch_ms session_duration_ms time_to_ready_ms time_to_first_frame_ms rebuffer_count rebuffer_duration_ms engine_switch_count rendered_frames dropped_frames discarded_failure_count state_duration_ms last_frame_age_ms current_buffer_ms");
const sessionKeys = new Set(["schema", "session_id", "capability_fingerprint", "final", "frames_known", "content_label", "content_key", "failure_http_statuses", "video_width", "video_height", "frame_rate", ...Object.keys(enums), ...Object.keys(lists), ...numbers]);
...
function playbackPayload(input, now = Date.now()) {
  if (!closed(input, new Set(["schema", "sampleId", "sampledAtMs", "device", "sessions"])) || input.schema !== 1 || typeof input.sampleId !== "string" || !UUID.test(input.sampleId)) return null;
  ...
  return { schema: 1, sampleId: input.sampleId.toLowerCase(), sampledAtMs: input.sampledAtMs, device, sessions };
}
